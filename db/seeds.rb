...
12.times do |j|
  date = "#{Time.zone.today.year}-#{j}-#{Time.zone.today.day}"
  100.times do |i|
    STDOUT.puts 'order'
    order = Order.create!(
    ...
    5.times do |j|
      ...
      product_id = Product.find_by(id: [rand(Product.ids.min..Product.ids.max)])
      ...
      OrderItem.create!(
        ...
        order_id: order.id,
        quantity: quantity,
        total_price: quantity * product_id.price,
        created_at: date,
        updated_at: date
      )
    end
